...
class ImprovedNNInferenceNode(Node):
    def __init__(self, pause_manager):
        ...
        self.action_timeout =2.0  # 30 seconds timeout for actions
        self.move_forward_action_timeout_multiplier = 2 # go for longer if move forward
        ...
        self.angular_speed = 1.0 # adjust to change turn spped
    ...
    def execute_action(self, action):
        ...
        print(f"Executing action: {action.action_type} with twist: linear={self.command.linear.x}, angular={self.command.angular.z}")
    ...
    def continue_action(self, action):
        ...
        self.publish_cmd_vel(self.command)
        if self.DEBUG:
            print(f"Continuing action: {action.action_type} with twist: linear={self.command.linear.x}, angular={self.command.angular.z}")
