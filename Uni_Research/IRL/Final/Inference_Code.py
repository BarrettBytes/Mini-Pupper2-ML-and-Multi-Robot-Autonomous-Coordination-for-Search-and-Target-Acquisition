...
import sys
import shutil
from Training_Code import DualNetworkPolicy 
...
class ImprovedNNInferenceNode(Node):
    ...
    def publish_cat_count(self):
        ...
        # Check if cat_count has reached 3
        if self.current_cat_count >= 3 and self.stop_on_three:
            print("Cat count reached 3! Stopping the robot and shutting down.")
            self.shutdown_node()
    ...
    def shutdown_node(self):
        ...
        # Launch a new terminal to publish /cat_count=3
        try:
            # Define the ROS2 publisher command
            # This command publishes "data: 3" to /cat_count at 1 Hz indefinitely
            ros2_pub_command = f'ros2 topic pub /cat_count std_msgs/msg/Int32 "data: 3" --rate 1'
            ...
            self.get_logger().info("Launched new terminal to publish /cat_count=3")
